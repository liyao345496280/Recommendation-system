    ...
    def prepare_context_Discrete_4context_short(self, Discrete_4context_short,data_file, negative_number=2, bert_true=True):
        with open(data_file, 'r', encoding='utf-8') as f:
            ...
            for line in tqdm(lines):
                ...
                template=template+'[SEP] Does the user accept or reject the response? [MASK]. [SEP] The short-term history: '+''.join(self.conti_tokens[1])+' <dialogue_history> '+'. [SEP] The current response: '+''.join(self.conti_tokens[0])+' <user_response> .'
                #[TASK] Predicting the action based on short-term dialogue history and current responses [SEP] Does the user accept or reject the response?[MASK] [SEP][P1][P2][P3][P4][P5][P6][P7][P8][P9][P10] The short-term history: <dialogue_history> [SEP] [Q1][Q2][Q3][Q4][Q5][Q6][Q7][Q8][Q9][Q10] the current response: <user_response>.
                ...
                base_sentence = template.replace("<dialogue_history>", prompt_context)
                #[TASK] Predicting the action based on short-term dialogue history and current responses. [SEP] Does the user accept or reject the response? [MASK]. [SEP] The short-term history: [P1][P2][P3][P4][P5][P6][P7][P8][P9][P10] user : i like funny movies. how about you [ns] .[SEP] The current response: [Q1][Q2][Q3][Q4][Q5][Q6][Q7][Q8][Q9][Q10] <user_response>.
                for _ in dialog['rec']:  # 若resp中有多个推荐的电影，则每一个推荐的电影对应的历史内容相同。
                    impid += 1
                    '''
                    a positive simple
    ...
    def prepare_context_Discrete_4context_long(self,Discrete_4context_long, data_file, negative_number=2, bert_true=True):
        with open(data_file, 'r', encoding='utf-8') as f:
            ...
            for line in tqdm(lines):
                ...
                template=template+'[SEP] Does the user accept or reject the response? [MASK]. [SEP] The long-term history: '+''.join(self.conti_tokens[1])+' <dialogue_history> '+'. [SEP] The current response: '+''.join(self.conti_tokens[0])+' <user_response> .'
