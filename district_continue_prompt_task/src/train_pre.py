...
if __name__ == '__main__':
    ...
    值得注意的是： data['context']和data['prompt']是dialog['context']+dialog['resp']的组合；而data['rec']是dialog['resp']中命名实体短语对应的实体id；并且dialog['resp']极可能是user说的话也可能是system说的话。
    '''
    train_dataset = CRSDataset(
        ...
        entity_max_length=args.entity_max_length,bert_tokenizer=bert_tokenizer,conti_tokens=conti_tokens
    )
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with torch.no_grad():
                ...
                # mask_hidden_states = [mask_hidden_states_discrete_relevance_short,
                #                       mask_hidden_states_discrete_relevance_long,
                #                       mask_hidden_states_discrete_unity_short,
                #                       mask_hidden_states_discrete_unity_long,
                #                       mask_hidden_states_discrete_action_short,
            ...
            loss_prompt_item_predict = model(**batch['prompt_item_predict']['context'], mask_prompt_hidden=mask_prompt_hidden,rec=True).rec_loss / args.gradient_accumulation_steps

            loss=loss_prompt_item_predict
        ...
        for batch in tqdm(valid_dataloader):
            with torch.no_grad():
                ...
                #                   mask_hidden_states_discrete_action_short.unsqueeze(1),
                #                   mask_hidden_states_discrete_action_long.unsqueeze(1)]
                mask_prompt_hidden = torch.cat(mask_hidden_states, dim=1)
                prompt_embeds = prompt_encoder(
                    entity_ids=batch['prompt_item_predict']['entity'],
                    token_embeds=token_embeds,
