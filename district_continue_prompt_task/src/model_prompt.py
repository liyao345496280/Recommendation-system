...
class KGPrompt(nn.Module):
    ...
    def forward(self, entity_ids=None, token_embeds=None, output_entity=False, use_rec_prefix=False,
        ...
        prompt_embeds = self.prompt_proj2(prompt_embeds)#torch.Size([64, 16, 768])# nn.Linear(hidden_size, n_layer * n_block * hidden_size)
        prompt_embeds = prompt_embeds.reshape(
            batch_size, prompt_len, self.n_layer, self.n_block, self.n_head, self.head_dim
        ).permute(2, 3, 0, 4, 1, 5)  # (n_layer, n_block, batch_size, n_head, prompt_len, head_dim)#(12，2，64，16，64) .。。。。。。这里reshape进行了广播torch.Size([64, 16, 12, 2, 12, 64])
