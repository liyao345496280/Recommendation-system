...
if __name__ == '__main__':
    ...
    #bert
    bert_tokenizer = AutoTokenizer.from_pretrained('../model/bert-base-uncased')
    bert_tokenizer.add_special_tokens(prompt_special_tokens_dict)
    '''
    bert:discrete
    '''
    answer = ['boring', 'interesting']
    special_template=template = "The user feels [MASK] about <candidate_news> according to his area of interest <user_sentence>"
    ...
    net4bert_discrete = discrete_Prompt(bert_tokenizer,model_name, answer_ids,mask_id).to(device)
    ...
    train_dataset = CRSDataset(
        ...
        entity_max_length=args.entity_max_length,bert_tokenizer=bert_tokenizer,key_name=args.key_name,special_template=special_template#key_name=='unity'
    )
    valid_dataset = CRSDataset(
        dataset=args.dataset, split='valid', tokenizer=tokenizer, debug=args.debug, max_length=args.max_length,
        prompt_tokenizer=text_tokenizer, prompt_max_length=args.prompt_max_length,
        entity_max_length=args.entity_max_length,bert_tokenizer=bert_tokenizer,key_name=args.key_name,special_template=special_template
    )
    ...
    data_collator = CRSDataCollator(
        ...
        prompt_tokenizer=text_tokenizer, prompt_max_length=args.prompt_max_length,bert_tokenizer=bert_tokenizer,key_name=args.key_name
    )
