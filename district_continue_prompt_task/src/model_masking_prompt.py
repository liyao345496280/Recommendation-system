...
class Fusion_KGPrompt(nn.Module):
    def __init__(
        ...
        super(Fusion_KGPrompt, self).__init__()
        ...
        self.relevance_prompt_proj =nn.Sequential(
            nn.Linear(token_hidden_size, token_hidden_size // 2),
            nn.ReLU(),
            nn.Linear(token_hidden_size // 2, token_hidden_size),
        )

        #self.LayerNorm=nn.LayerNorm([60, token_hidden_size])
        self.LayerNorm = nn.LayerNorm([90,token_hidden_size])
    ...
    def forward(self, entity_ids=None, token_embeds=None, output_entity=False, use_rec_prefix=False,
                use_conv_prefix=False,mask_prompt_hidden=None):
        ...
        if token_embeds is not None:
            ...
            token_embeds = self.token_proj2(token_embeds)  # torch.Size([batch_size, 200, 768])
        ...
        if entity_embeds is not None and token_embeds is not None:
            attn_weights = self.cross_attn(token_embeds) @ entity_embeds.permute(0, 2,
                                                                                 1)  # (batch_size, token_len, entity_len) -->#torch.Size([32, 200, 768])*torch.Size([32, 768, entity_len])=torch.Size([64, 200, entity_len])
            attn_weights /= self.hidden_size  # torch.Size([64, 200, 16])

            if output_entity:
                token_weights = F.softmax(attn_weights, dim=1).permute(0, 2, 1)  # torch.Size([64, 16, 200])
                prompt_embeds = token_weights @ token_embeds + entity_embeds  # torch.Size([64, 16, 768])
                #prompt_embeds=entity_embeds
                prompt_len = entity_len  # 16
            else:
        ...
        #     prefix_embeds = self.rec_prefix_proj(self.rec_prefix_embeds) + self.rec_prefix_embeds  # 可学习的提示向量
        ...
        #     prefix_embeds = self.conv_prefix_proj(self.conv_prefix_embeds) + self.conv_prefix_embeds  # 可学习的提示向量
        #     prefix_embeds = prefix_embeds.expand(prompt_embeds.shape[0], -1, -1)
        #     prompt_embeds = torch.cat([prefix_embeds, prompt_embeds], dim=1)
        #     prompt_len += self.n_prefix_conv

        if mask_prompt_hidden is not None:
           mask_prompt_hidden=self.relevance_prompt_proj(self.LayerNorm(mask_prompt_hidden))
           prompt_embeds = torch.cat([mask_prompt_hidden, prompt_embeds], dim=1)
           prompt_len += mask_prompt_hidden.shape[1]


        prompt_embeds = self.prompt_proj1(prompt_embeds) + prompt_embeds
        prompt_embeds = self.prompt_proj1(prompt_embeds)
        prompt_embeds = self.prompt_proj2(
            prompt_embeds)  # torch.Size([64, 16, 768])# nn.Linear(hidden_size, n_layer * n_block * hidden_size)
        prompt_embeds = prompt_embeds.reshape(
            batch_size, prompt_len, self.n_layer, self.n_block, self.n_head, self.head_dim
        ).permute(2, 3, 0, 4, 1,
                  5)  # (n_layer, n_block, batch_size, n_head, prompt_len, head_dim)#(12，2，64，16，64) .。。。。。。这里reshape进行了广播torch.Size([64, 16, 12, 2, 12, 64])
